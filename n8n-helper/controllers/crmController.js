...
module.exports = {
    ...
    processMessage: async (req, res) => {
        ...
        try {
            ...
            const configRows = await query(`SELECT title, value FROM atc_config WHERE title LIKE 'crm_%timer%'`);
            const timerConfig = {};
            if (configRows && Array.isArray(configRows)) configRows.forEach(row => { timerConfig[row.title] = parseInt(row.value); });
            ...
            if (!leads || leads.length === 0) {
                const leadResult = await query('INSERT INTO crm_leads (created, source_platform, first_interaction_type) VALUES (NOW(), ?, ?)', [platform, 'message']);
            ...
            await query('UPDATE crm_leads SET last_interaction_time = NOW() WHERE id = ?', [leadId]);
            ...
            const activeTicket = openTickets && Array.isArray(openTickets) ? openTickets.find(t => {
            ...
            if (activeTicket) {
                ...
                await query(`UPDATE crm_tickets SET score = ?, updated = NOW() WHERE id = ?`, [currentScore, ticketId]);
            } else if (shouldCreateTicket) {
                const typeRes = await query(`SELECT id FROM crm_ticket_types WHERE code = ? LIMIT 1`, [ticketCode]);
                const typeId = typeRes && typeRes.length > 0 ? typeRes[0].id : 46; // 46 = GENERAL_ENQUIRY fallback
            ...
            if (ticketId) {
                ...
                await query(`UPDATE crm_ticket_timers SET status = 'cancelled' WHERE crm_ticket_id = ? AND status = 'pending'`, [ticketId]);
                await query(`INSERT INTO crm_ticket_timers (crm_ticket_id, timer_type, scheduled_at, status) VALUES (?, ?, DATE_ADD(NOW(), INTERVAL ? MINUTE), 'pending')`, [ticketId, timerType, timerMinutes]);
            }
