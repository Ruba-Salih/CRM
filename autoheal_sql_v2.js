...
if (q24) {
  q24.parameters.query = `SET @conversation_id = NULL;
SET @fb_conv_id = NULL;
SET @lead_id = NULL;
SET @fallback_lead_id = NULL;
...
INSERT IGNORE INTO facebook_conversations(facebook_conversation_id, lead_id, message_count, last_message_time) 
SELECT COALESCE(NULLIF(NULLIF('{{ $json.conversation_id }}', 'undefined'), ''), CONCAT('fb_temp_', REPLACE(UUID(),'-',''))), @fallback_lead_id, 1, '{{ new Date($('Message Preparer6').item.json.created_time).toISOString().slice(0, 19).replace('T', ' ') }}'
WHERE @conversation_id IS NULL AND @fallback_lead_id IS NOT NULL;
...
if (q25) {
  q25.parameters.query = `SET @conversation_id = NULL;
SET @fb_conv_id = NULL;
SET @lead_id = NULL;
SET @fallback_lead_id = NULL;
...
INSERT IGNORE INTO facebook_conversations(facebook_conversation_id, lead_id, message_count, last_message_time) 
SELECT COALESCE(NULLIF(NULLIF('{{ $json.conversation_id }}', 'undefined'), ''), CONCAT('fb_temp_', REPLACE(UUID(),'-',''))), @fallback_lead_id, 1, NOW()
WHERE @conversation_id IS NULL AND @fallback_lead_id IS NOT NULL;
...
console.log('Fixed SQL queries to clear session variables and handle undefined conversation_id');
