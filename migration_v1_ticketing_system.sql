-- ==========================================================
-- CRM Ticketing System Migration - Version 1.0
-- Description: Adding platforms, refactoring messages, and timer systems
-- ==========================================================

START TRANSACTION;

-- 1. Create crm_platforms Table
...
INSERT IGNORE INTO `crm_platforms` (`code`, `name`) VALUES
('facebook', 'Facebook'),
('instagram', 'Instagram'),
('whatsapp', 'WhatsApp'),
('telegram', 'Telegram'),
...
-- 3. Modify crm_ticket_messages (New Normalized Structure)
...
ALTER TABLE `crm_ticket_messages`
  ADD COLUMN `platform_id` int(11) DEFAULT NULL AFTER `crm_ticket_id`,
  ADD COLUMN `interaction_type` enum('message', 'comment') DEFAULT 'message' AFTER `platform_id`,
  ...
  ADD COLUMN `router_value` varchar(50) DEFAULT NULL AFTER `message_text`,
...
-- 4. Update crm_tickets with Sales Context and Timing
...
-- 7. Insert New Ticket Types

-- Updated ticket types: 8 for "Out of Plan" and 8 for "New Course Request"

INSERT INTO `crm_ticket_types` (`id`, `code`, `name_en`, `name_ar`) VALUES
...
-- 8. Add Timer Settings to atc_config
